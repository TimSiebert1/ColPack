 ...
 *  @brief class GraphColoring in @link group1@endlink.
 ...
 Graph coloring is an assignment of consecutive integral numbers (each
 representing a color) to vertices, edges or faces or a combination of two or
 more of these objects of a graph such that it satisfes one or more constraints.
 The present version of ColPack provides methods for vertex coloring only. The
 minimum number of vertex colors required to color a graph is known as the
 chromatic number of the graph. The problem of finding the chromatic number for
 even a planar graph is NP-hard. ColPack features some of the most efficient
 approximation algorithms available to date for some of the vertex coloring
 problems.
 */
class GraphColoring : public GraphOrdering {
public: // DOCUMENTED
  /// Return the Seed matrix based on existing coloring. This Seed matrix is
  /// managed and freed by ColPack
  /** Precondition:
  - the Graph has been colored

  Postcondition:
  - Size of the returned matrix is (*ip1_SeedRowCount) rows x
  (*ip1_SeedColumnCount) columns.
  (*ip1_SeedRowCount) == num of columns of the original matrix ==
  GetVertexCount()
  (*ip1_SeedColumnCount) == num of colors used to color vertices ==
  GetVertexColorCount().

  Notes:
  - This Seed matrix is managed and automatically freed by ColPack when the
  Graph object is deleted. Therefore, the user should NOT attempt to free the
  Seed matrix again.
  */
  double **GetSeedMatrix(int *ip1_SeedRowCount, int *ip1_SeedColumnCount);

  /// Same as GetSeedMatrix(), except that this Seed matrix is NOT managed by
  /// ColPack
  /** Notes:
  - This Seed matrix is NOT managed by ColPack. Therefore, the user should free
  the Seed matrix manually when the matrix is no longer needed.
  */
  double **GetSeedMatrix_unmanaged(int *ip1_SeedRowCount,
                                   int *ip1_SeedColumnCount);

  /// Quick check to see if DistanceTwoColoring() ran correctly
  /**
  Return value:
  - 1 when this function detects that DistanceTwoColoring() must have run
  INcorrectly.
  - 0 otherwise

  IMPORTANT: This is the quick check so if CheckQuickDistanceTwoColoring()
  return 1, then DistanceTwoColoring() definitely ran INcorrectly. However, when
  CheckQuickDistanceTwoColoring() return 0, it doesn't mean that
  DistanceTwoColoring() ran correctly (it may, it may not). To be 100% sure, use
  CheckDistanceTwoColoring()

  Precondition: DistanceTwoColoring() has been run.

  Parameter: int Verbose
  - If Verbose == 0, this function only check and see if m_i_MaximumVertexDegree
  <= m_i_VertexColorCount + 1.
  - If Verbose == 1, this function will print out the vertex with
  m_i_MaximumVertexDegree where the error can be detected.
  - If Verbose == 2, this function will print out all the errors (violations)
  and then return.
  ...
  */
  ...
  /** 100% accurate but slow. For a quick check, use
  ...
  Parameter: int Verbose
  - If Verbose == 0, this function will silently return after the first error is
  detected.
  - If Verbose == 1, this function will print out the error message and return
  after the first error is detected.
  - If Verbose == 2, this function will print out all the errors and then
  return.
  */
...
private:
  ...
  int FindCycle(int, int, int, int, vector<int> &, vector<int> &,
                vector<int> &);

  // Private Function 1402
  int UpdateSet(int, int, int, map<int, map<int, int>> &, vector<int> &,
...
public:
   ...
   * - If D1 neighbor has color id > D2 neighbor's color id, then that D2
   * neighbor's color is forbidden (the current vertex cannot use that color)
  ...
  /*
   * Related paper: A. Gebremedhin, A. Tarafdar, F. Manne and A. Pothen, New
   * Acyclic and Star Coloring Algorithms with Applications to Hessian
   * Computation, SIAM Journal on Scientific Computing, Vol 29, No 3, pp
   * 1042--1072, 2007.
  ...
  /**
   * NOTE: At this point, this routine will not work correctly if there are
   * conflicts
   */
  int BuildStarCollection(vector<int> &vi_VerticesToBeRecolored);
  int PrintStarCollection(vector<int> &vi_EdgeStarMap,
                          vector<int> &vi_StarHubMap,
                          map<int, map<int, int>> &mimi2_VertexEdgeMap);

  struct lt_pii {
    bool operator()(const pair<int, int> pii_ColorCombination1,
                    const pair<int, int> pii_ColorCombination2) const {
  ...
  struct Colors2Edge_Value {
    Colors2Edge_Value() { visited = false; }
    vector<pair<int, int>> value;
    bool visited;
  };
  /// Build the collection of 2-color star from the coloring result
  /**
   * This function also helps us identify a list of vertices need to be
   * recolored if conlict is detected If vi_VerticesToBeRecolored.size() == 0,
   * then the coloring is a valid star coloring. The algorithm is done in
   * parallel
   */
  int DetectConflictInColorCombination(
      int i_MaxNumThreads, int i_thread_num,
  ...
  int BuildStarFromColorCombination(
      int i_MaxNumThreads, int i_thread_num,
      pair<int, int> pii_ColorCombination,
      map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
      map<int, vector<pair<int, int>>> *Vertex2ColorCombination_Private,
   ...
   * After Vertex2ColorCombination is built, Vertex2ColorCombination_Private
   * will be deallocated
   */
  int BuildVertex2ColorCombination(
      int i_MaxNumThreads,
      map<int, vector<pair<int, int>>> *Vertex2ColorCombination_Private,
   ...
   *
   * Return values:
   * - >= 0: Fail. the vertex that causes conflict as this routine progress.
   * Note: this may not be the latest-added vertex that cause coloring conflict
   * in the graph
   * - -2: Fail. 2 potential hub are connected
   * - -1: Pass.
   *
   * If pii_ConflictColorCombination is provided (i.e.
   * pii_ConflictColorCombination!=NULL) and this Check fail,
   * pii_ConflictColorCombination will contain the 2 problematic colors
   */
  int CheckStarColoring_OMP(int i_Mode,
                            pair<int, int> *pii_ConflictColorCombination);
  int BuildStarFromColorCombination_forChecking(
      int i_Mode, int i_MaxNumThreads, int i_thread_num,
      pair<int, int> pii_ColorCombination,
      map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
      map<int, int> *PotentialHub_Private);
  int BuildForbiddenColors(int i_MaxNumThreads, int i_thread_num,
                           int i_CurrentVertex,
                           map<int, bool> *mip_ForbiddenColors,
                           map<int, int> *D1Colors,
                           vector<map<int, int>> *Vertex2ColorCombination);
  int PrintVertex2ColorCombination(
      vector<map<int, int>> *Vertex2ColorCombination);
  int PrintVertex2ColorCombination_raw(
      vector<map<int, int>> *Vertex2ColorCombination);
  int PrintVertexAndColorAdded(int i_MaxNumThreads,
                               vector<pair<int, int>> *vi_VertexAndColorAdded,
                               int i_LastNEntries = 999999999);
  int PrintForbiddenColors(map<int, bool> *mip_ForbiddenColors,
                           int i_thread_num);
  int PickVerticesToBeRecolored(int i_MaxNumThreads,
                                vector<pair<int, int>> *ConflictedEdges_Private,
                                vector<int> &ConflictCount);
  int PrintAllColorCombination(
      map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
      int i_MaxNumThreads, int i_MaxNumOfCombination = 1000000,
      int i_MaxElementsOfCombination = 100000);
  int PrintColorCombination(
      map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
      int i_MaxNumThreads, pair<int, int> pii_ColorCombination,
      int i_MaxElementsOfCombination = 100000);
  int PrintPotentialHub(map<int, int> *PotentialHub_Private, int i_thread_num,
                        pair<int, int> pii_ColorCombination);
  int PrintConflictEdges(vector<pair<int, int>> *ConflictedEdges_Private,
                         int i_MaxNumThreads);
  ...
  /** Sample code:
          ...
          delete graph;
   */
  int BuildSubGraph(map<int, map<int, bool>> *graph, int i_CenterVertex,
                    int distance = 1,
                    map<int, bool> *mib_FilterByColors = NULL);

  /** Sample code: (see function int BuildSubGraph() )
   */
  int BuildConnectedSubGraph(map<int, map<int, bool>> *graph,
                             int i_CenterVertex, int distance = 1,
                             map<int, bool> *mib_FilterByColors = NULL);
  ...
  /** Sample code:
          ...
          delete graph;
   */
  int BuildColorsSubGraph(map<int, map<int, bool>> *graph,
