 ...
 *  @brief class GraphOrdering in @link group1@endlink.
 ...
 This class stores the ordered vertices as a vector of vertex identifiers to be
 used by coloring methods.
 */
class GraphOrdering : public GraphInputOutput {
public:
  /// Calculate and return the Maximum Back degree
  /**
  Precondition: OrderVertices() has been called, i.e. m_vi_OrderedVertices has
  been populated Note: Back degree of a vertex is the degree of that vertex in
  the subgraph consisting of vertices that had been ordered (i.e., the vertices
  that are ordered before the current vertex). Depend on the ordering style,
  each vertex in vector m_vi_OrderedVertices may have different Back degree. The
  Maximum Back degree of all vertices in the graph will be returned. This is the
  UPPER BOUND for the number of colors needed to D1-color the graph.
  //*/
  int GetMaxBackDegree();

  int OrderVertices(string s_OrderingVariant);

  /// Test and make sure that the ordering is valid. Return 0 if the ordering is
  /// invalid, 1 if the ordering is valid.
  ...
  Actually make a call to "bool isValidOrdering(vector<int> & ordering, int
  offset = 0);"
  */
...
protected:
  ...
  vector<int> m_vi_OrderedVertices; // m_vi_OrderedVertices.size() =
                                    // m_vi_Vertices.size() - 1
