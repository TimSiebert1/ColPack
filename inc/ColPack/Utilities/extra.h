...
- int i_mode:
  ...
  The next 3 parameter(s) are: unsigned int ** uip2_SparsityPattern, int
i_Matrix_Row, int i_Matrix_Col
  - i_mode == 1: output the structure of the matrix and values in the Compressed
Matrix The next 6 parameter(s) are: unsigned int ** uip2_SparsityPattern, int
i_Matrix_Row, int i_Matrix_Col, double** dp2_CompressedMatrix, int
i_CompressedMatrix_Row, int i_CompressedMatrix_Col
  - i_mode == 2: output the structure and values of the matrix and values in the
Compressed Matrix The next 7 parameter(s) are: unsigned int **
uip2_SparsityPattern, int i_Matrix_Row, int i_Matrix_Col, double**
dp2_CompressedMatrix, int i_CompressedMatrix_Row, int i_CompressedMatrix_Col,
double** dp2_Values
*/
int WriteMatrixMarket_ADOLCInput(string s_postfix, int i_mode, ...);

/// Convert a number string under Harwell-Boeing format to the format that C++
/// can understand
/** For example: -6.310289677458059D-07 to -6.310289677458059E-07

Essentially, this function just search backward for the letter D and replace it
with E
...
int buildDotWithoutColor(ColPack::GraphColoringInterface &g,
                         vector<string> &ListOfColors, string fileName);

/// Build dot file with colors, also highlight StarColoringConflicts
/**
 * !!! TO DO: improve this function so that it can detect conflicts of all
 * coloring types
 */
...
Line 2-(# of non-zeros + 1): <# of non-zeros in that row> <index of the 1st
non-zero> <index of the 2nd non-zero> ... <index of the (# of non-zeros in that
row)th non-zero>
*/
int ReadRowCompressedFormat(string s_InputFile,
                            unsigned int ***uip3_SparsityPattern, int &rowCount,
                            int &columnCount);

/// Test and make sure that this is a valid ordering.
/** This routine will test for:
- Duplicated vertices. If there is no duplicated vertex, this ordering is
probably ok.
...
/// Convert all the characters in input to upper case, ' ', '\ t', '\ n' will be
/// converted to '_'
string toUpper(string input);

/// Build the index struture from Row Compressed Format to Sparse Solvers Format
/**
ip2_RowIndex and ip2_ColumnIndex will be allocated memory (using malloc) and
populated with the matrix structure in Sparse Solvers Format
...
int ConvertRowCompressedFormat2SparseSolversFormat_StructureOnly(
    unsigned int **uip2_HessianSparsityPattern, unsigned int ui_rowCount,
    unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex);

/// Convert Coordinate Format to Row Compressed Format
/**
dp3_Pattern and dp3_Values will be allocated memory (using malloc) and populated
with the matrix structure in Row Compressed Format
...
*/
int ConvertCoordinateFormat2RowCompressedFormat(
    unsigned int *uip1_RowIndex, unsigned int *uip1_ColumnIndex,
    double *dp1_HessianValue, int i_RowCount, int i_NonZeroCount,
    unsigned int ***dp3_Pattern, double ***dp3_Values);
...
Note: DIMACS graph format is for directed graph => the equivalent matrix is
squared and non-systemic
...
/// Read the sparse matrix from Matrix-Market-format file and convert to Row
/// Compressed format (used by ADIC) "uip3_SparsityPattern" & "dp3_Value"
/** Read in a matrix from matrix-market format file and create a matrix stored
in compressed sparse row format The Matrix-Market-format has 3 values in each
row, the row index, column index and numerical value of each nonzero. The last 4
parameters of this routine are output parameters (unsigned int ***
uip3_SparsityPattern, double*** dp3_Value,int &rowCount, int &columnCount)
*/
int ConvertMatrixMarketFormat2RowCompressedFormat(
    string s_InputFile, unsigned int ***uip3_SparsityPattern,
    double ***dp3_Value, int &rowCount, int &columnCount);
...
//*/
/// Convert Row Compressed format (used by ADOL-C) to zero-based indexing,
/// 3-array variation CSR format (used by ADIC)
/**
Return 0 upon successful.
*/
// !!! need to be fixed to accomodate dp2_Value parameter
int ConvertRowCompressedFormat2CSR(
    unsigned int **uip2_SparsityPattern_RowCompressedFormat, int i_rowCount,
    int **ip_RowIndex, int **ip_ColumnIndex);

int ConvertRowCompressedFormat2ADIC(
    unsigned int **uip2_SparsityPattern_RowCompressedFormat, int i_rowCount,
    double **dp2_Value, std::list<std::set<int>> &lsi_SparsityPattern,
    std::list<std::vector<double>> &lvd_Value);

/// Multiply the original sparse matrix (uip3_SparsityPattern,dp3_Value) (in
/// compress sparse row format) with the seed matrix dp2_seed and store the
/// result in "dp3_CompressedMatrix"
/** (*dp3_CompressedMatrix) = (*dp3_Value) * dp2_seed
 */
int MatrixMultiplication_VxS(unsigned int **uip3_SparsityPattern,
                             double **dp3_Value, int rowCount, int columnCount,
                             double **dp2_seed, int colorCount,
                             double ***dp3_CompressedMatrix);

int MatrixMultiplication_VxS__usingVertexPartialColors(
    std::list<std::set<int>> &lsi_SparsityPattern,
    std::list<std::vector<double>> &lvd_Value, int columnCount,
    vector<int> &vi_VertexPartialColors, int colorCount,
    double ***dp3_CompressedMatrix);

/// Multiply the seed matrix dp2_seed with the original sparse matrix
/// (uip3_SparsityPattern,dp3_Value) (in compress sparse row format) and store
/// the result in "dp3_CompressedMatrix"
...
/**
        If (compare_exact == 0) num1 and num2 are consider equal if 0.99 <=
   num1/num2 <= 1.02 If (print_all == 1) all cases of non-equal will be print
   out. Normally (when print_all == 0), this rountine will stop after the first
   non-equal.
*/
bool CompressedRowMatricesAreEqual(double **dp3_Value, double **dp3_NewValue,
                                   int rowCount, bool compare_exact = 1,
                                   bool print_all = 0);

bool ADICMatricesAreEqual(std::list<std::vector<double>> &lvd_Value,
                          std::list<std::vector<double>> &lvd_NewValue,
                          bool compare_exact = 1, bool print_all = 0);

/// just manipulate the value of dp2_Values a little bit. Each non-zero entry in
/// the matrix * 2 + 1.5.
...
int GenerateValues(unsigned int **uip2_SparsityPattern, int rowCount,
                   double ***dp3_Value);

/// Allocate memory and generate random values for dp3_Value of a Symmetric
/// Matrix.
int GenerateValuesForSymmetricMatrix(unsigned int **uip2_SparsityPattern,
                                     int rowCount, double ***dp3_Value);
...
int displayGraph(map<int, map<int, bool>> *graph,
                 ...
                 int i_RunInBackground = false, int filter = DOT);
...
int displayGraph(T &g, int i_RunInBackground = false, int filter = DOT) {
  ...
  if (m_s_VertexColoringVariant.empty() ||
      m_s_VertexColoringVariant == "Unknown") {
  ...
  case FDP:
    ...
    break;
  default:
    command = "xdot -f dot "; // case DOT
  }

  command = command + fileName;
  if (i_RunInBackground)
    command = command + " &";
  int i_ReturnValue = system(command.c_str());
  return i_ReturnValue;
}

/// Find the difference between 2 arrays. Return 0 if there is no difference, 1
/// if there is at least 1 difference
template <class T>
int diffArrays(T *array1, T *array2, int rowCount, bool compare_exact = 1,
               bool print_all = 0) {
  double ratio = 0.;
  ...
  for (int i = 0; i < rowCount; i++) {
    if (compare_exact) {
      if (array1[i] != array2[i]) { // found a difference
        cout << "At index i=" << i << "\t array1[] = " << array1[i] <
            ";\t array2[] = " << array2[i] << endl;
        none_equal_count++;
        if (!print_all)
          return 1;
      }
...
/// Find the difference between 2 vectors. Return 0 if there is no difference, 1
/// if there is at least 1 difference
template <class T>
int diffVectors(vector<T> array1, vector<T> array2, bool compare_exact = 1,
                bool print_all = 0) {
  double ratio = 0.;
  int none_equal_count = 0;

  if (array1.size() != array2.size()) {
    cout << "array1.size() " << array1.size() << " != array2.size()"
         << array2.size() << endl;
    none_equal_count++;
  }

  int min_array_size =
      (array1.size() < array2.size()) ? array1.size() : array2.size();
...
template <class T> int freeMatrix(T **xp2_matrix, int rowCount) {
...
template <class T> int freeMatrix(T ***xp3_matrix, int rowCount) {
...
template <class T> int deleteMatrix(T **xp2_matrix, int rowCount) {
  // cout<<"IN deleteM 2"<<endl<<flush;
  // printf("* deleteMatrix rowCount=%d \n",rowCount);
  // Pause();
  for (int i = 0; i < rowCount; i++) {
...
template <class T> int deleteMatrix(T ***xp3_matrix, int rowCount) {
  // cout<<"IN deleteM 3"<<endl<<flush;
...
template <class T>
void displayCompressedRowMatrix(T **xp2_Value, int rowCount,
                                bool structureOnly = false) {
  unsigned int estimateColumnCount = 20;
  cout << setw(4) << "[" << setw(3) << "\\" << "]       ";
  if (structureOnly) {
    for (unsigned int j = 0; j < estimateColumnCount; j++)
      cout << setw(4) << j;
  } else {
    for (unsigned int j = 0; j < estimateColumnCount; j++)
      cout << setw(9) << j;
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++) {
    ...
    if (structureOnly) {
      for (unsigned int j = 1; j <= numOfNonZeros; j++)
        cout << setw(4) << (int)xp2_Value[i][j];
      // for(unsigned int j=1; j <= numOfNonZeros; j++) {
      //   printf("  %d",(int)xp2_Value[i][j]);
      // }
    } else {
      for (unsigned int j = 1; j <= numOfNonZeros; j++)
        cout << setw(9) << (float)xp2_Value[i][j];
      // for(unsigned int j=1; j <= numOfNonZeros; j++) {
...
template <class T>
void displayMatrix(T **xp2_Value, int rowCount, int columnCount,
                   bool structureOnly = false) {
  cout << setw(4) << "[" << setw(3) << "\\" << "]";
  if (structureOnly) {
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++) {
    ...
    if (structureOnly) {
      for (unsigned int j = 0; j < (unsigned int)columnCount; j++)
        cout << setw(3) << (bool)xp2_Value[i][j];
    } else {
      for (unsigned int j = 0; j < (unsigned int)columnCount; j++)
        printf("  %7.2f", (float)xp2_Value[i][j]);
      // for(unsigned int j=0; j < (unsigned int)columnCount; j++)
      // cout<<setw(8)<<xp2_Value[i][j];
    }
    cout << endl << flush;
  }
  cout << endl << endl;
...
template <class T> int displayVector(vector<T> v) {
...
template <class T>
void displayAdjacencyMatrix(vector<vector<T>> &xp2_Value,
                            bool structureOnly = false) {
  unsigned int estimateColumnCount = 20;
  cout << setw(4) << "[" << setw(3) << "\\" << "]";
  if (structureOnly) {
