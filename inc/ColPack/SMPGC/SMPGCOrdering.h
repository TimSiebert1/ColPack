...
public: // construction
  SMPGCOrdering(const string &file_name, const string &fmt, double *iotime,
                const string &order, double *ordtime);
...
public: // API: global ordering
  ...
  const vector<int> &global_ordered_vertex() const {
    return m_global_ordered_vertex;
  }
  const string &global_ordered_method() const {
    return m_global_ordered_method;
  }
...
protected: // API: local ordering
  ...
  void local_largest_degree_first_ordering(vector<int> &vtxs, const int beg,
                                           const int end);
