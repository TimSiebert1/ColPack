...
#include <ColPack/ColPackHeaders.h> //#include "GraphOrdering.h"
...
// [2] Grah coloring algorithms for multi-core and massively multithreaded
// architectures [3] A Parallel Graph Coloring Heuristic
//=============================================================================
...
public: // Constructions
  ...
  SMPGCColoring(const string &graph_name, const string &fmt,
                double *iotime = nullptr, const string &ord = "NATURAL",
                double *ordtime = nullptr);
...
public: // API
  ...
  void get_vertex_colors(vector<int> &x) {
    x.assign(m_vertex_color.begin(), m_vertex_color.end());
  }
  ...
  int D1_OMP_GM3P(int nT, int &color, vector<int> &vtxColors,
                  const int local_order = ORDER_NONE);
  ...
  int D1_OMP_JP(int nT, int &color, vector<int> &vtxColors,
                const int local_order = ORDER_NONE);
  int D1_OMP_MTJP(int nT, int &color, vector<int> &vtxColors,
                  const int local_order = ORDER_NONE);
  int D1_OMP_HBJP(int nT, int &color, vector<int> &vtxColors,
                  const int option = HYBRID_SERIAL, const int swtich_iter = 0,
                  const int local_order = ORDER_NONE);
  int D1_OMP_HBMTJP(int nT, int &color, vector<int> &vtxColors,
                    const int option = HYBRID_SERIAL, const int switch_iter = 0,
                    const int local_order = ORDER_NONE);

  int D1_OMP_GM3P_BIT(int nT, int &color, vector<int> &vtxColors,
                      const int local_order = ORDER_NONE);
  int D1_OMP_GMMP_BIT(int nT, int &color, vector<int> &vtxColors,
                      const int local_order = ORDER_NONE);
  ...
  inline void hybrid_GM3P(const int nT, vector<int> &vtxColors,
                          vector<vector<int>> &Q,
                          const int local_order = ORDER_NONE);
  inline void hybrid_GMMP(const int nT, vector<int> &vtxColors,
                          vector<vector<int>> &Q,
                          const int local_order = ORDER_NONE);
  inline void hybrid_Serial(vector<int> &vtxColors, vector<vector<int>> &Q,
                            const int local_order = ORDER_NONE);
